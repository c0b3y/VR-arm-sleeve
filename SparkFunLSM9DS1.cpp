...
void LSM9DS1::calibrate(bool autoCalc)
{
        ...
        while (samples < 0x1F)
        {
                samples = (xgReadByte(FIFO_SRC, settings.device.bus) & 0x3F); // Read number of stored samples
        }
...
{
        return (xgReadByte(FIFO_SRC, settings.device.bus) & 0x3F);
}
...
uint8_t LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress, uint8_t bus)
{
   // printf("Starting BMA180 I2C sensor state byte read\n");
    ...
    int file;
    if ((file = open(namebuf, O_RDWR)) < 0){
            printf("Failed to open BMA180 Sensor %s on I2C Bus\n", namebuf);
    }
    if (ioctl(file, I2C_SLAVE, address) < 0){
            printf("I2C_SLAVE address %c failed...\n", address);
    }
